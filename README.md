...
> **状态：2026-09-08 环境与指标修复已完成 CPU 验证；新版教师和训练仍待完成。**
...
**最新规划：2026-09-07-r2。** 保留 4B/T1/T2 架构，先修证据输入、教师与评价，
再做强同协议比较。公共价值策略监督是核心；显式 VoI 辅助监督是匹配增强臂，
ECRG 检验风险—收益，外部迁移优先一个 ExCyTIn 环境。

**本轮代码更新：** 响应物理效果与意图标签分离，背叛检测要求相关证据，
Canary 改为仿真注入与延迟遥测，修正工具成本、终止条件和指标分母。
已通过 **130 项 CPU 测试与 8 个场景 smoke**；不代表 GPU 稳定性或模型效果已验证。
环境/指标版本分别为 `aep-env-20260908-v1`、`aep-metrics-20260908-v1`。
输入修正、公共价值教师和完整因果评测仍待完成，详见[当前状态](docs/STATUS.md)。
下方旧结果原样保留，**不是本轮修复后的结果，也不能直接跨版本比较**。

## 从这里阅读

| 文档 | 内容 |
|---|---|
| **[整体规划](docs/PLAN.md)** | 最新收敛方案：必要修正、失败诊断、两条匹配训练、强基线、因果/迁移及执行门槛 |
...
不是已经完成无捷径验证的实现。
最新 CPU 反例还确认了部分旧引用证据丢失、来源依赖/时间信息编码碰撞；
它们尚未修复，历史发生率也尚未量化，详见状态页。
...
- 本轮包含环境与指标语义修复，但未启动新训练；剩余门禁明确列在状态页。
