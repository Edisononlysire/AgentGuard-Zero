...
def _chat_template_kwargs() -> dict:
    value = os.environ.get("AGZ_DISABLE_THINKING", "0").strip().lower()
    return {"enable_thinking": False} if value in {"1", "true", "yes", "on"} else {}


def _has_multimodal_payload(non_tensor_batch: Dict[str, Any]) -> bool:
    """Check for actual image/video values, not just a dataset placeholder key."""
    value = non_tensor_batch.get("multi_modal_data")

    def has_value(item: Any) -> bool:
        if item is None:
            return False
        if isinstance(item, np.ndarray):
            return any(has_value(value) for value in item.tolist())
        if isinstance(item, dict):
            return any(has_value(value) for value in item.values())
        if isinstance(item, (list, tuple, set)):
            return any(has_value(value) for value in item)
...
def _global_any_active(active_mask: torch.Tensor) -> bool:
    """Keep FSDP agent ranks in the same turn loop until all trajectories finish."""
    local_active = bool(active_mask.any().item())
    if not torch.distributed.is_initialized() or torch.distributed.get_world_size() <= 1:
        return local_active
    flag = torch.tensor([int(local_active)], dtype=torch.int32)
...
class AgentActorManager:
    def __init__(
        ...
        if self.config.mtrl_sep is None:
            messages = [{"role": self.config.mtrl_role, "content": "{obs}"}]
            try:
                ...
                # Some model templates require an explicit user query. MTRL is
                # optional, so keep worker initialization model-agnostic.
                rendered = f"{self.config.mtrl_role}: {{obs}}\nassistant:"
        ...
        if self.processor:
            try:
                ...
                self.mm_prefix, marker, self.mm_postfix = rendered.partition("|||")
                if not marker:
                    raise ValueError("processor chat template removed the text marker")
            except Exception:
                # Text-only AgentGuard trajectories do not require multimodal
                # wrappers, even when AutoProcessor is available for the model.
                self.mm_prefix = ""
                self.mm_postfix = ""
        else:
            self.mm_prefix = ""
    ...
    async def _process_next_obs(self, next_obs: List[str], dones: List[bool], valid_action: List[bool], finishs: List[bool], tool_interact_info: List[dict], rollings: DataProto) -> Tuple[torch.Tensor, List[dict]]:
        ...
        """
        has_multi_modal_data = _has_multimodal_payload(rollings.non_tensor_batch)
        mm_data_list = None
        async with self.tokenizer_lock:
                ...
                for k, tool_interact_info_k in enumerate(tool_interact_info):
                    try:
                        ...
                        if content_list and not dones[k] and not finishs[k]:
                            next_obs_message = [{"role": self.config.mtrl_role, "content": content_list}]
    ...
    async def run_llm_loop_async(self, gen_batch: DataProto, **sampling_params: Dict[str, Any]) -> Tuple[Dict, Dict]:
        ...
        if _has_multimodal_payload(gen_batch.non_tensor_batch) and self.config.rollout_mode != "async":
        ...
        if _has_multimodal_payload(rollings.non_tensor_batch):
        ...
        for step in range(self.config.max_turns):
            ...
            if not _global_any_active(active_mask):
                break
            ...
            if not local_has_active:
                # Another rank still has a live trajectory. Generate one dummy
                # continuation so every FSDP rank executes identical collectives.
                selection_mask = torch.zeros_like(active_mask)
                selection_mask[0] = True
            rollings_active = DataProto.from_dict(
                {k: v[selection_mask] for k, v in rollings.batch.items()},
                {k: v[selection_mask.numpy()] for k, v in rollings.non_tensor_batch.items()},
            ...
            next_obs, dones, valid_action, finishs, rewards, tool_interact_info = await self.interact_with_tool_server(
                ...
                is_last_step=(step + 1 >= self.config.max_turns)
            )
            for i, reward in enumerate(rewards):
                if rewards[i] is not None and active_mask[i]:
                    turns_stats_extra["rewards"][i].append(reward)
