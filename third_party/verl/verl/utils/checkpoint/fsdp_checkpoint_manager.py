...
def _generation_config_for_checkpoint(model, model_config):
    """Return a saveable generation config even when the model has no config file."""
    ...
    if name_or_path:
        try:
            return GenerationConfig.from_pretrained(name_or_path)
        except OSError:
            logger.warning(
    ...
    def save_checkpoint(self, local_path: str, hdfs_path: str = None, global_step: int = 0, max_ckpt_to_keep=None):
        ...
        if self.rank == 0:
            ...
            if unwrap_model.can_generate():
                generation_config = _generation_config_for_checkpoint(unwrap_model, model_config)
