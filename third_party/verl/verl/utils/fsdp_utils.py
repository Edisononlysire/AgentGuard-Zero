...
def layered_summon_lora_params(fsdp_module) -> OrderedDict:
    ...
    prefix_list = [
        # Qwen3.5 conditional-generation models keep decoder layers below
        # language_model, unlike their text-only config-built counterpart.
...
def require_nonempty_lora_params(lora_params: OrderedDict) -> OrderedDict:
    """Reject adapter exports that silently collected no trainable tensors."""
    if not lora_params:
        raise RuntimeError("LoRA adapter export collected zero tensors")
    return lora_params
