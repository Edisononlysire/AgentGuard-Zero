...
def _configure_worker_triton_cache() -> None:
    root = os.environ.get("AGZ_TRITON_CACHE_ROOT", f"/tmp/agentguard_zero_triton_{os.environ.get('USER', 'user')}")
    namespace = os.environ.get("AGZ_TRITON_CACHE_NAMESPACE") or os.environ.get("VERL_RUN_ID", "verl")
    safe_namespace = "".join(
        char if char.isalnum() or char in "-_." else "_" for char in namespace
    )
    rank = os.environ.get("RANK")
    worker_key = f"rank_{rank}" if rank is not None else f"pid_{os.getpid()}"
    cache_dir = os.path.join(root, safe_namespace, worker_key)
    os.makedirs(cache_dir, exist_ok=True)
    os.environ["TRITON_CACHE_DIR"] = cache_dir


def _distributed_timeout() -> timedelta:
    seconds = int(os.environ.get("VERL_NCCL_TIMEOUT_SECONDS", "7200"))
    if seconds <= 0:
        raise ValueError("VERL_NCCL_TIMEOUT_SECONDS must be positive")
    return timedelta(seconds=seconds)


_configure_worker_triton_cache()
...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def _build_model_optimizer(
        ...
        try:
            ...
            # Transformers 5 renamed the vision-to-text auto class.
        ...
        # Transformers 5 attaches `_is_hf_initialized` metadata to Qwen3.5
        # parameters. Accelerate's meta-device registrar forwards that private
        # attribute into torch.nn.Parameter, which fails on nonzero FSDP ranks.
        # Loading both ranks on CPU avoids the incompatible constructor path;
        # FSDP still shards the unchanged weights before training.
        use_meta_tensor = (
            not actor_model_config.tie_word_embeddings
            and getattr(actor_model_config, "model_type", None) != "qwen3_5"
        )
        ...
        if (
            ...
            # Gemma can hang with nested FSDP during HF rollout. Qwen3.5 needs
            # its declared decoder-layer policy to avoid a full-model flat
            # buffer and shard clone exceeding a 40 GB device during init.
        ...
        # Actor CPUOffload stays opt-in because it can be slower and may interact
        # poorly with gradient accumulation, but it is useful for constrained
        # smoke runs where FSDP init would otherwise exceed GPU memory.
        if role == "actor":
    ...
    def _build_hf_full_rollout_replica(self, trust_remote_code=False):
        """Build an unsharded inference-only copy for Qwen3.5 HF generation."""
        ...
        replica = model_class.from_pretrained(
            local_path,
        ...
        replica = get_peft_model(replica, lora_config)
        replica.requires_grad_(False)
        replica.eval()
        return replica

    def _sync_hf_full_rollout_replica(self):
        lora_params = require_nonempty_lora_params(layered_summon_lora_params(self.actor_module_fsdp))
        result = set_peft_model_state_dict(self.rollout.module, lora_params, adapter_name="default")
        unexpected = list(getattr(result, "unexpected_keys", []))
        if unexpected:
            raise RuntimeError(f"HF rollout replica rejected LoRA keys: {unexpected[:8]}")
        return len(lora_params)
    ...
    def _build_rollout(self, trust_remote_code=False):
        ...
        if rollout_name == "hf":
            ...
            self._hf_full_rollout_replica = bool(
                self.config.rollout.get("use_full_replica", False)
                and self._is_lora
                and getattr(self.actor_model_config, "model_type", None) == "qwen3_5"
            )
            rollout_module = (
                self._build_hf_full_rollout_replica(trust_remote_code=trust_remote_code)
                if self._hf_full_rollout_replica
                else self.actor_module_fsdp
            )
            rollout = HFRollout(module=rollout_module, config=self.config.rollout, tokenizer=self.tokenizer)
    ...
    def generate_sequences(self, prompts: DataProto):
        ...
        with self.rollout_sharding_manager:
            ...
            use_full_replica = getattr(self, "_hf_full_rollout_replica", False)
            if use_full_replica:
                with simple_timer("hf_rollout_lora_sync", timing_generate):
                    lora_tensor_count = self._sync_hf_full_rollout_replica()
                    self.rollout.activate(get_device_id())
                if self.rank == 0:
                    print(f"Synced {lora_tensor_count} LoRA tensors to HF rollout replica", flush=True)
            try:
                with simple_timer("generate_sequences", timing_generate):
                    output = self.rollout.generate_sequences(prompts=prompts)
            finally:
                if use_full_replica:
                    self.rollout.deactivate()
        ...
        if self._is_lora and hasattr(getattr(self, "actor_module", self.actor_module_fsdp), "peft_config"):
            ...
            try:
                if fsdp_version(self.actor_module_fsdp) > 0:
                    ...
                    lora_params = require_nonempty_lora_params(
                        layered_summon_lora_params(self.actor_module_fsdp)
                    )
                    if dist.get_rank() == 0:
