...
mkdir -p "${ROOT}/logs" "${ROOT}/outputs/checkpoints"

export AGZ_ROOT="${ROOT}"
source "${ROOT}/scripts/qwen35_env.sh"

MODEL_SIZE=${AGZ_QWEN35_SIZE:-4B}
case "${MODEL_SIZE}" in
  4B)
    export AGZ_MODEL_PATH="${AGZ_MODEL_PATH:-${AGZ_QWEN35_4B_PATH}}"
    ;;
  9B)
    export AGZ_MODEL_PATH="${AGZ_MODEL_PATH:-${AGZ_QWEN35_9B_PATH}}"
    ;;
  *)
    echo "Unsupported AGZ_QWEN35_SIZE=${MODEL_SIZE}; expected 4B or 9B." >&2
    exit 65
    ;;
esac

export AGZ_RUN_NAME="${AGZ_RUN_NAME:-agentguard_vda_qwen35_${MODEL_SIZE}_lora_${BATCH_JOB_ID:-manual}}"
...
export AGZ_MAX_STEPS="${AGZ_MAX_STEPS:-20}"
...
export AGZ_LORA_TARGET_MODULES="${AGZ_LORA_TARGET_MODULES:-[q_proj,k_proj,v_proj,o_proj,gate_proj,up_proj,down_proj]}"
