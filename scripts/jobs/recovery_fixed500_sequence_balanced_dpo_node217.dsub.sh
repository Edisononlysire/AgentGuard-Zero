...
COMMON="$BASE/teacher_sft_sequence_balanced_cap4"
PREFERENCES="$BASE/preferences_balanced"
OUT="$BASE/teacher_sft_sequence_balanced_cap4_plus_dpo_aligned"
EVAL="$BASE/evaluations/teacher_sft_sequence_balanced_cap4_plus_dpo_aligned_quick8_nothink"
...
if (
    ...
    raise SystemExit("aligned sequence-balanced parent adapter is not accepted")
PY
...
for shard in 0 1 2 3; do
  cache="${AGZ_TRITON_CACHE_ROOT}/fixed500_seq_dpo/eval_shard_${shard}"
  mkdir -p "$cache"
  TRITON_CACHE_DIR="$cache" CUDA_VISIBLE_DEVICES="${GPU_IDS[$shard]}" python \
    scripts/eval_recovery_fixed_source.py run \
    --variant teacher_sft_sequence_balanced_cap4_plus_dpo_aligned_quick8_nothink \
...
echo "RECOVERY_FIXED500_SEQUENCE_DPO_READY=$OUT"
