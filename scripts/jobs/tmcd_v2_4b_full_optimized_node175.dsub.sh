...
# DCA: 80 prompts x 2 rollouts x 25 steps = the unchanged 4,000 feedback pool.
...
# GPU rollout measured maxima are 180 action tokens and 789 observation tokens.
...
# Match the stable 9B VDA schedule: 32 trajectories globally and at most
# eight active rollouts per GPU. The full 2,400-example epoch is unchanged.
...
# HF rollout can use expandable CUDA segments; this reduces allocator
# fragmentation across repeated long multi-turn generations.
export AGZ_PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True
# Retain only the latest recovery snapshot, once per third of the round.
...
python -s "${ROOT}/scripts/preflight_tmcd_v2_job.py" \
  ...
  --output "${ROOT}/outputs/tmcd_v2/preflight/node175_4b_full_optimized.json"
...
touch "${ROOT}/outputs/tmcd_v2/node175_4b_full_optimized.SUCCEEDED"
