...
EXPECTED_NODE=${AGZ_GATE_EXPECTED_NODE:-cyclone001-agent-217}
if [[ "$(hostname)" != "${EXPECTED_NODE}" ]]; then
  echo "Refusing to run outside ${EXPECTED_NODE}: $(hostname)" >&2
  exit 72
fi

OUT=${AGZ_GATE_OUTPUT_DIR:-${ROOT}/outputs/tmcd_v2/gates/vda-full9-b32}
if [[ -e "${OUT}" ]]; then
...
export AGZ_TRAIN_FILE=${AGZ_GATE_TRAIN_FILE:-${ROOT}/data/tmcd_v2/qwen3.5-9b/round_1/vda_train/train.parquet}
export AGZ_VAL_FILE=${AGZ_GATE_VAL_FILE:-${ROOT}/data/tmcd_v2/qwen3.5-9b/round_1/vda_dev/dev.parquet}
export AGZ_RUN_NAME=${AGZ_GATE_RUN_NAME:-agz_gate_qwen3.5-9b_vda_batch32_prompt_v5}
...
export AGZ_REF_PARAM_OFFLOAD=true
...
python -s "${ROOT}/scripts/finalize_vda_training_gate.py" \
  ...
  --pool-manifest "${AGZ_GATE_POOL_MANIFEST:-${ROOT}/data/tmcd_v2/qwen3.5-9b/round_1/vda_pool_manifest.json}" \
