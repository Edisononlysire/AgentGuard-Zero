...
OUT="${ROOT}/outputs/optimization_gates/vda-delta-wire-v3-vllm-batch64-gate"
if [[ -e "${OUT}" ]]; then
...
with source.open(encoding="utf-8") as handle:
    for line in handle:
        ...
        if len(scenarios) == 64:
            break
if len(scenarios) < 64:
    raise SystemExit(f"need 64 unique valid TMCD-v2 scenarios, found {len(scenarios)}")
...
export AGZ_RUN_NAME=agz_gate_qwen3.5-4b_vda_vllm_batch64
...
export AGZ_BATCH_SIZE=64
export AGZ_PPO_MINI_BATCH_SIZE=32
export AGZ_ROLLOUT_N=1
export AGZ_ROLLOUT_BACKEND=vllm
