...
DATA_ROOT=${ROOT}/data/tmcd_v242/qwen3.5-4b
CHECKPOINT_ROOT=${ROOT}/checkpoints/tmcd_v242/qwen3.5-4b
...
# Preserve 4,000 DCA feedback examples and 100 optimizer mini-updates.
...
# TMCD v2.4 feedback semantics are part of the formal protocol lock.
...
# Formal VDA protocol budgets from the completed four-task HF gate.  Each
# generation batch is split into three 32-scenario PPO mini-updates, preserving
# all 2,400 scenarios and 75 optimizer updates per round.
...
python -s "${ROOT}/scripts/preflight_tmcd_v2_job.py" \
  ...
  --output "${ROOT}/outputs/tmcd_v242/preflight/node175_4b_full.json"
...
touch "${ROOT}/outputs/tmcd_v242/node175_4b_full.SUCCEEDED"
