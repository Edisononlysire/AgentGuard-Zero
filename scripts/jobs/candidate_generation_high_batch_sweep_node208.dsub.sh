#!/bin/bash
#DSUB -n AGZ4BCandidateHighSweep
#DSUB -N 1
...
OUT="${ROOT}/outputs/optimization_gates/candidate-generation-high-batch-sweep"
if [[ -e "${OUT}" ]]; then
  echo "Refusing to overwrite existing sweep: ${OUT}" >&2
  exit 73
fi
mkdir -p "${OUT}"
export AGZ_TRITON_CACHE_ROOT="/tmp/agentguard_zero_candidate_high_sweep_${USER}"
