...
GATE_ROOT="${ROOT}/outputs/tmcd_v2/gates/vda_batch160_dynamic_20260715"
GATE_DATA="${GATE_ROOT}/train.parquet"
mkdir -p "${GATE_ROOT}" "${ROOT}/logs/tmcd_v2_optimized"

# Reproduce the failed formal batch. Short contexts may use 40 sequences/GPU;
# the HF token budget automatically splits long contexts before the OOM point.
python -s -c 'import pyarrow.parquet as pq, sys; table=pq.read_table(sys.argv[1]).slice(800, 160); assert table.num_rows == 160; pq.write_table(table, sys.argv[2])' \
...
export AGZ_RUN_NAME=agz_tmcd_v2_full_qwen3.5-4b_vda_batch160_dynamic_gate
...
export AGZ_HF_MAX_BATCH_TOKENS=196608
