...
GATE_ROOT="${ROOT}/outputs/tmcd_v2/gates/vda_batch80_20260715"
GATE_DATA="${GATE_ROOT}/train.parquet"
mkdir -p "${GATE_ROOT}" "${ROOT}/logs/tmcd_v2_optimized"

# Exercise a formal long-tail region with 80 trajectories globally (20/GPU).
python -s -c 'import pyarrow.parquet as pq, sys; table=pq.read_table(sys.argv[1]).slice(800, 80); assert table.num_rows == 80; pq.write_table(table, sys.argv[2])' \
...
export AGZ_RUN_NAME=agz_tmcd_v2_full_qwen3.5-4b_vda_batch80_gate
