...
ADAPTER="$ROOT/outputs/recovery/fixed500_threeway_20260719/teacher_sft_balanced/adapter"
OUT="$ROOT/outputs/recovery/fixed500_threeway_20260719/evaluations/teacher_sft_balanced_quick8_nothink"
...
for shard in 0 1 2 3; do
  cache="${AGZ_TRITON_CACHE_ROOT}/fixed500_quick8_nothink/shard_${shard}"
  mkdir -p "$cache"
  TRITON_CACHE_DIR="$cache" CUDA_VISIBLE_DEVICES="${GPU_IDS[$shard]}" python \
    scripts/eval_recovery_fixed_source.py run \
    --variant teacher_sft_balanced_quick8_nothink --model-path "$MODEL" \
    --adapter "$ADAPTER" --scenarios "$XPLAY" --device cuda:0 \
    --scenario-limit 8 --skip-teacher-diagnostics \
...
echo "RECOVERY_FIXED500_QUICK8_READY=$OUT"
