...
OUT="$BASE/teacher_sft_compact_intent_3ep"
EVAL="$BASE/evaluations/teacher_sft_compact_intent_3ep_quick8"
...
python -m torch.distributed.run --standalone --nproc_per_node=4 \
  ...
  --min-records 2317 --max-records 2317 --epochs 3.0 \
...
for shard in 0 1 2 3; do
  cache="${AGZ_TRITON_CACHE_ROOT}/fixed500_intent/eval_shard_${shard}"
  mkdir -p "$cache"
  TRITON_CACHE_DIR="$cache" CUDA_VISIBLE_DEVICES="${GPU_IDS[$shard]}" python \
    scripts/eval_recovery_fixed_source.py run \
    --variant teacher_sft_compact_intent_3ep_quick8 \
...
echo "RECOVERY_FIXED500_COMPACT_INTENT_READY=$OUT"
