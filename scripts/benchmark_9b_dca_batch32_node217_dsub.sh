...
GATE_ROOT="${ROOT}/outputs/optimization_gates/qwen3.5-9b-dca-b32-m4"
PROMPTS="${GATE_ROOT}/prompts.parquet"
mkdir -p "${GATE_ROOT}"
python -s - "${PROMPTS}" <<'PY'
import sys
from agentguard_zero.training.dca_dataset import write_dca_prompt_dataset
write_dca_prompt_dataset(sys.argv[1], num_rows=32, seed=20260709, backbone="qwen3.5-9b", source_round=0)
PY
...
export AGZ_RUN_NAME=agz_gate_qwen3.5-9b_dca_b32_m4
